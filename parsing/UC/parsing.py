...
class Parsing_Class(Paths_Class):
    ...
    def string_and_split(self, l):
        ...
        start_date_time = dt.datetime.combine(start_date_object.date(), start_time_object.time())
        ...
        end_date_time = dt.datetime.combine(end_date_object.date(), end_time_object.time())
    ...
    def parsing_evnt(self, l, os_used='native'):
        with open(l, 'r') as f:
            if f:
                ...
                if '.af.uchicago.edu' in first_line or 'workers-ads-long-' in first_line and N<=1:
                    try:
                ...
                #  if the host-name is not in the first line continue
                    ...
                    elif '.af.uchicago.edu' in last_line or 'workers-ads-long-'in last_line:
                        host_name=last_line
                        payload_size=int(0)
                        exit_code=int(1)
                    ...
                    # Gets:  end time, run time, host-name, payload size, exit code(0), and creates dic.
                        ...
                        host_name = self.checks_for_split_logs(l)[2]
                        payload_size = int(self.checks_for_split_logs(l)[3].split('\t')[0])
                        exit_code=int(0)
                        dic = self.dictionary_maker(start_date_time_timestamp, queue_time, run_time, payload_size, exit_code, host_name)
            else:
                print("ERROR -- FILE WAS NOT OPENED")
        ...
        with open(l, 'r') as f:
            if f:
                ...
                try:
                    ...
                    start_date_object = dt.datetime(int(start_date[0:4]), int(start_date[5:7]), int(start_date[8:10]))
                    start_date_time = dt.datetime.combine(start_date_object.date(), start_time_object.time())
                    start_date_time_timestamp = int(start_date_time.replace(tzinfo=timezone.utc).timestamp()*1e3)
                    end_time_object = dt.datetime.strptime(end_time, '%H:%M:%S')
                    end_date_object = dt.datetime(int(end_date[0:4]), int(end_date[5:7]), int(end_date[8:10]))
                    end_date_time = dt.datetime.combine(end_date_object.date(), end_time_object.time())
                    run_time=int((end_date_time-start_date_time).total_seconds())
                    dic = self.dictionary_maker(start_date_time_timestamp, queue_time, run_time, payload_size, exit_code, host_name)
                except IndexError:
                    try:
                        ...
                        day=int(dateTimeString[8:10])
                        hour=int(dateTimeString[11:13])
                        start_date_time = dt.datetime(year,month, day, hour, int(0), int(0))
                        start_date_time_timestamp = int(start_date_time.replace(tzinfo=timezone.utc).timestamp()*1e3)
                        run_time=int(0)
                        payload_size=int(0)
                        dic = self.dictionary_maker(start_date_time_timestamp, queue_time, run_time, payload_size, exit_code, host_name)
