...
class Parsing_Class:
    ...
    job_dictionary: ClassVar[dict] = {
        ...
        "EVNT_container_el": "EVNT-el9-container-batch",
        "EVNT_container_centos": "EVNT-centos7-container-batch",
        "TRUTH3_centos7_container": "truth3-centos-container-batch",
    }
    ...
    def benchmark_path(self):
        benchmark_paths = [
            os.path.join(self.site_dir, i) for i in sorted(os.listdir(self.site_dir))
        ]
    ...
    def parsing_truth3(
        ...
        with open(log_path) as f:
            if f:
                ...
                submit_time_datetime_object = dt.datetime(
                    year,
                    month,
                    day,
                    int(submit_time_list[0]),
                    int(submit_time_list[1]),
                    int(submit_time_list[2]),
                )
                start_time_datetime_object = dt.datetime(
                    year,
                    month,
                    day,
                    int(start_time_list[0]),
                    int(start_time_list[1]),
                    int(start_time_list[2]),
                )
                start_date_time_timestamp = int(
                    (start_time_datetime_object.timestamp()) * 1e3
                )
                # Obtains the queue time
                queue_time = int(
                    (
                        start_time_datetime_object - submit_time_datetime_object
                    ).total_seconds()
                )
                end_time_line_list = file_lines[N - 3].split(" ")
                end_time_list = end_time_line_list[0].split(":")
                end_time_datetime_object = dt.datetime(
                    year,
                    month,
                    day,
                    int(end_time_list[0]),
                ...
                run_time = int(
                    (
                        end_time_datetime_object - start_time_datetime_object
                    ).total_seconds()
                )
    ...
    def parsing_truth3_e1(
        ...
        with open(log_path) as f:
            if f:
                ...
                dic[self.dic_keys[3]] = 0
                dic[self.dic_keys[4]] = 0
                dic[self.dic_keys[5]] = 0
                dic[self.dic_keys[6]] = 1
    ...
    def parsing_truth3_log_split(
        ...
        with open(log_path) as f:
            if f:
                ...
                submit_time_object = dt.datetime(
                    year,
                    month,
                    day,
                    int(submit_time_list[0]),
                ...
                if batch:
                    ...
                    queue_time = int(
                        (start_time_object - submit_time_object).total_seconds()
                    )
                else:
                    start_time_timestamp = int(
                        (start_time_object.replace(tzinfo=timezone.utc).timestamp())
                        * 1e3
                    )
                    queue_time = 0
    ...
    def parsing_truth3_interactive(
        ...
        with open(log_path) as f:
            if f:
                ...
                start_date_time_timestamp = int(
                    (
                        start_time_datetime_object.replace(
                            tzinfo=timezone.utc
                        ).timestamp()
                    )
                    * 1e3
                ...
                if "DAOD_TRUTH3.TRUTH3.root" in payload_line_list:
                    ...
                    payload_size = 0
                    exit_code = 1
                end_datetime_object = dt.datetime(
                    year,
                    month,
                    day,
                    int(end_time_string[0:2]),
                    int(end_time_string[3:5]),
                    int(end_time_string[6:8]),
                )
                run_time = int(
                    (end_datetime_object - start_time_datetime_object).total_seconds()
                )
    ...
    def missing_log_file(self, l):
        ...
        end_time = self.checks_for_split_logs(l)[1]
        end_time_h = int(end_time[0:2])
        end_time_m = int(end_time[3:5])
        end_time_s = int(end_time[6:8])
        end_datetime = dt.datetime(year, month, day, end_time_h, end_time_m, end_time_s)
        ...
        payload_size = int(self.checks_for_split_logs(l)[3])
        if payload_size != 0:
            exit_code = 0
        else:
            exit_code = 1
    ...
    def fnf_parsing_evnt(self, l, os_used="native"):
        try:
            split_file = l.replace(self.log_name, self.split)
            with open(split_file) as g:
                if g:
                    file_lines = g.read().splitlines()
                    exit_code = 1
                    host_name = file_lines[-2]
                    payload_size = int(file_lines[-1].split("\t")[0])
                    datetimeString = log_path.split("/")[6]
                    year = int(datetimeString[0:4])
                    month = int(datetimeString[5:7])
                    day = int(datetimeString[8:10])
            ...
            with open(l) as g:
                if g:
                    ...
                    # Checks the length of the first line to confirm indices
                    if n == 8:
    ...
    def new_parsing_evnt(self, l, os_used="native"):
        with open(log_path) as f:
            if f:
                file_lines = f.read().splitlines()
                N = len(file_lines)
                ...
                if "sdfampere" in first_line and N != 1:
                ...
                elif "sdfampere" in first_line and N == 1:
                    host_name = first_line
                    payload_size = 0
                    queue_time = 0
                    datetimeString = log_path.split("/")[6]
                    ...
                    last_line = file_lines[N - 1]
                    if "EVNT" in last_line:
                        host_name = file_lines[N - 2]
                        payload_size = int(last_line.split("\t")[0])
                        ...
                        exit_code = 1
                        split_log_lines = self.checks_for_split_logs(l)
                        end_time = last_line.split(" ")[0]
                        end_time_h = int(end_time[0:2])
                        end_time_m = int(end_time[3:5])
                        end_time_s = int(end_time[6:8])
                        ...
                        run_time = int((end_datetime - start_datetime).total_seconds())
                        if "EVNT" not in split_log_lines:
                            host_name = split_log_lines[-1]
                            payload_size = 0
                        dic = self.dictionary_maker(
                            start_date_time_timestamp,
                            queue_time,
                        ...
                        run_time = int((end_datetime - start_datetime).total_seconds())
                        split_log_lines = self.checks_for_split_logs(l)
                        payload_size = int(split_log_lines[-1].split("\t")[0])
                        host_name = split_log_lines[-2]
                        exit_code = 0
                        dic = self.dictionary_maker(
                            start_date_time_timestamp,
                            queue_time,
        ...
        with open(log_path) as f:
            if f:
                ...
                if first_line == last_line:
                    ...
                    start_datetime_object = dt.datetime(year, month, day, hour, 0, 0)
                    start_date_time_timestamp = int(
                        start_datetime_object.replace(tzinfo=timezone.utc).timestamp()
                        * 1e3
                    )
                elif N == 2:
                    host_name = first_line
                    ...
                    start_date_time_timestamp = int(
                        start_time_object.replace(tzinfo=timezone.utc).timestamp() * 1e3
                    )
    ...
    def parsing_ntuple_c(self, log_path, fli=7):
        with open(log_path) as f:
            if f:
                ...
                start_datetime_object = dt.datetime(
                    year, start_month, start_day, start_hour, start_min, start_sec
                )
                ...
                end_datetime_object = dt.datetime(
                    year, end_month, end_day, end_hour, end_min, end_sec
                )
                run_time = int(
                    (end_datetime_object - start_datetime_object).total_seconds()
                )
                host_name = file_lines[N - 2]
                payload_size = 0
                exit_code = 0
    ...
    def parsing_ntuple_c_e1(self, log_path):
        with open(log_path) as f:
            if f:
                ...
                date_time_string = start_date_time_line_list[6]
    ...
    def parsing_ntuple_c_e3(self, log_path, fli=7):
        with open(log_path) as f:
            if f:
                file_lines = f.read().splitlines()
                N = len(file_lines)
                ...
                payload_size = 0
                lines_dates = []
                for log_path in file_lines:
                    if "2025-" in l:
                        lines_dates.append(l)
                n = len(lines_dates)

                start_date = lines_dates[0].split(" ")[0]
                start_time = lines_dates[0].split(" ")[1]
                ...
                end_time = lines_dates[n - 1].split(" ")[1]
