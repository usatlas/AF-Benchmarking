...
class Parsing_Class:
    ...
    def dictionary_maker(self, start_date_time_timestamp, queue_time, run_time, payload_size, exit_code, host_name):
        # Creates a dictionary with predetermined keys
        dic = dict.fromkeys(self.dic_keys)
        # Assigns values to the keys
        dic[self.dic_keys[0]] = self.af_dictionary[self.site]
    ...
    def missing_log_file(self, l):
        ...
        start_datetime=dt.datetime(year, month, day, start_time_h,start_time_m,start_time_s)
        start_date_time_timestamp = int(start_datetime.replace(tzinfo=timezone.utc).timestamp()*1e3)
        ...
        if payload_size !=0:
            ...
            exit_code=int(1)
        dic=self.dictionary_maker(start_date_time_timestamp, queue_time, run_time, payload_size, exit_code, host_name)
        return dic
        ...
        try:
            ...
            with open(l, 'r') as g:
                if g:
                    ...
                    start_datetime=dt.datetime(year, month, day, start_time_h,start_time_m,start_time_s)
                    submit_datetime=dt.datetime(year, month, day, submit_time_h,submit_time_m,submit_time_s)
        ...
        with open(l, 'r') as f:
            if f:
                ...
                if 'sdfampere' in first_line and N!=1:
                    ...
                    start_datetime=dt.datetime(year,month,day,hour, int(0), int(0))
                    start_date_time_timestamp = int(start_datetime.replace(tzinfo=timezone.utc).timestamp()*1e3)
                    run_time=int(0)
                    exit_code=int(1)
                    queue_time=int(0)
    ...
    def parsing_ntuple_c_e3(self, l, fli=7):
        with open(l, 'r') as f:
            if f:
                ...
                start_datetime=dt.datetime(year, month, day, int(start_time[0:2]), int(start_time[3:5]), int(start_time[6:8]))
                start_date_time_timestamp = int(start_datetime.replace(tzinfo=timezone.utc).timestamp()*1e3)
                ...
                end_datetime=dt.datetime(year, month, day, int(end_time[0:2]), int(end_time[3:5]), int(end_time[6:8]))
                run_time = int((end_datetime - start_datetime).total_seconds())
                exit_code=int(1)
                host_name=file_lines[N-1]
                 # Creates a dictionary with predetermined keys
