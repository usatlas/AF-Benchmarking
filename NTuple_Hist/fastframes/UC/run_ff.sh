...
cp ~/AF-Benchmarking/NTuple_Hist/fastframes/UC/example_config.yml ${working_dir}
...
source ${ATLAS_LOCAL_ROOT_BASE}/user/atlasLocalSetup.sh -c el9 -m /data/ -r "asetup StatAnalysis,0.5.0 &&\
  ...
  pip3 install pyyaml &&\
  date >> split.log &&\
  python3 /data/$(whoami)/ntuple_hist/FastFrames/python/FastFrames.py -c ${working_dir}/example_config.yml 2>&1 | tee ff.log"
