...
def full_path_function(list_of_paths):
    ...
    for i in list_of_paths:
        ...
        if "TRUTH3_el9_container" in sorted(os.listdir(i)):
            dir_with_job_name = os.path.join(i, "TRUTH3_el9_container")
            name_of_log_file = "log.Derivation"
            if name_of_log_file in sorted(os.listdir(dir_with_job_name)):
                list_of_full_path.append(
                    os.path.join(dir_with_job_name, name_of_log_file)
                )
            else:
                continue
    ...
    if f:
        ...
        try:
            ...
            time_diff_queue = dt.datetime.strptime(
                start_time, "%H:%M:%S"
            ) - dt.datetime.strptime(submit_time, "%H:%M:%S")
...
def bookkeeping_data(list_of_jsons):
    ...
    if "truth3_el9_batch_sent.txt" in os.listdir(python_script_dir):
        f = open("truth3_el9_batch_sent.txt")
        formatted_lines = []
        if f:
        ...
        f = open("truth3_el9_batch_sent.txt", "w")
        if f:
...
def append_new_data(diff_set):
    f = open("truth3_el9_batch_sent.txt", "a")
    if f:
...
def saving_data_as_tuple(list_of_dics, list_of_full_path, list_of_jsons):
    ...
    f = open("truth3_el9_batch_job_log.txt", "w")
    if f:
