...
def full_path_function(list_of_paths):
    ...
    for i in list_of_paths:
        ...
        if "EVNT" in os.listdir(i):
            dir_with_job_name = os.path.join(i, "EVNT")
...
def bookkeeping_data(list_of_jsons):
    ...
    if 'evnt_native_sent.txt' in os.listdir(python_script_dir):
        instances_not_in_file = []
        f = open('evnt_native_sent.txt', 'r')
        formatted_lines = []
        if f:
...
def saving_data_as_tuple(list_of_dics, list_of_full_path, list_of_jsons):
    ...
    f = open("evnt_native_job_log.txt", "w")
    if f:
