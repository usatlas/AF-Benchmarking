...
def full_path_function(list_of_paths):
    ...
    for i in list_of_paths:
        ...
        if "EVNT_contained_centos7" in sorted(os.listdir(i)):
            dir_with_job_name = os.path.join(i, "EVNT_contained_centos7")
...
def parsing_function(element_from_list_of_full_path):
    ...
    if f:
        ...
        try:
            ...
            # Gets the queue time
            time_diff_queue = dt.datetime.strptime(time_started, "%H:%M:%S") - dt.datetime.strptime(time_submitted, "%H:%M:%S")
            queue_time = int(time_diff_queue.total_seconds())
            # Constructs the initial datetime object
            start_datetime_object = dt.datetime(int(year_submitted), int(month_submitted), int(day_submitted), int(time_started[0:2]), int(time_started[3:4]), int(time_started[6:8]))
            last_line = lines[N-3]
            last_line_list = last_line.split(" ")
            # Gets the time completed from the last line list
            time_completed = last_line_list[0]
            running_time_diff = dt.datetime.strptime(time_completed, "%H:%M:%S") - dt.datetime.strptime(time_started, "%H:%M:%S")
            ...
            queue_time = int(0)
            run_time = int(0)
            start_datetime_object = dt.datetime(int(year_submitted), int(month_submitted), int(day_submitted), int(time_started), int(0), int(0))
        except ValueError:
            last_line = lines[N-2]
            last_line_list = last_line.split(" ")
            time_completed = last_line_list[0]
            ...
            # Gets the run+queue time 
            run_time = int(running_time_diff.total_seconds()) + queue_time 
            host_machine = payload_size_line[:-1]
            payload_size = int(0)
...
def creates_dictionaries(list_of_parsed_data, af_location='uc'):
    ...
    for tuples in list_of_parsed_data:
        ...
        dic["testType"] = "EVNT-centos7-container-batch"
...
def bookkeeping_data(list_of_jsons):
    ...
    if 'evnt_centos7_sent.txt' in os.listdir(python_script_dir):
        instances_not_in_file = []
        f = open('evnt_centos7_sent.txt', 'r')
        formatted_lines = []
        if f:
        ...
        f = open('evnt_centos7_sent.txt', 'w')
        if f:
...
def append_new_data(diff_set):
    python_script_dir = r"/Users/selbor/Work/env/"
    f = open('evnt_centos7_sent.txt', 'a')
    if f:
...
def saving_data_as_tuple(list_of_dics, list_of_full_path, list_of_jsons):
    ...
    f = open("evnt_centos7_job_log.txt", "w")
    if f:
