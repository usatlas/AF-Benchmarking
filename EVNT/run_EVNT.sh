...
Container(){
  ...
  source ${ATLAS_LOCAL_ROOT_BASE}/user/atlasLocalSetup.sh -c ${1} -r ${2}"asetup AthGeneration,23.6.31,here && \
  pwd Gen_tf.py --ecmEnergy=13000.0 --jobConfig=${3}  --outputEVNTFile=EVNT.root --maxEvents=10000 --randomSeed=${4}"
}
...
main() {
  ...
  then
    ...
    local  OScontainer="centos7"
    local  mount="/srv"
    Container ${OScontainer} ${mount} ${config_dir} ${seed}
  elif [[ -d /data ]]
  then
