...
if "rpv" in phys_short.lower() and "import " not in process:
    raise RuntimeError(
...
if evt_multiplier > 0:
    ...
    # Sensible default
    nevts = evgenConfig.nEventsPerJob * 2.0
...
if not hasattr(runArgs, "inputGeneratorFile") or runArgs.inputGeneratorFile is None:
    ...
    if tarfile.is_tarfile(runArgs.inputGeneratorFile):
        ...
        update_lhe_file(
            lhe_file_old=runArgs.inputGeneratorFile,
            param_card_old=param_card_old,
            masses=masses,
        )
...
if njets > 0 and njets != njets_min and hasattr(genSeq, "Pythia8"):
    genSeq.Pythia8.Commands += [
        ...
        f"Merging:nQuarksMerge = {_nQuarksMerge:d}",
    ]
