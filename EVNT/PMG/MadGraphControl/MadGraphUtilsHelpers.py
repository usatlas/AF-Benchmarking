...
def settingIsTrue(setting):
    if setting.replace("'", "").replace('"', "").replace(".", "").lower() in [
        "t",
        "true",
    ]:
        return True
...
def is_version_or_newer(args):
    ...
    version = None
    version_file = open(os.environ["MADPATH"] + "/VERSION")
    ...
    # this is lazy, let's hope there won't be a subversion > 100...
...
def get_physics_short():
    ...
    jofiles = [
        f for f in os.listdir(FIRST_DIR) if (f.startswith("mc") and f.endswith(".py"))
    ]
    if len(jofiles) == 0:
